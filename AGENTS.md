...
- Safe multisig: `blocowallet safe import <network> <name> <checksummed-address>` verifies owners/threshold on-chain and imports a custody-free account; proposals are created with `safe propose`, approved by owner accounts with `safe sign`, and executed with `safe execute`. The TUI "Safe multisig" menu reviews and approves pending proposals. Hardware owners cannot yet sign raw Safe digests (no raw-digest device flow); software and cloud owners are supported.
