...
- Safe multisig: `blocowallet safe import <network> <name> <checksummed-address>` verifies owners/threshold on-chain and imports a custody-free account; proposals are created with `safe propose`, approved by owner accounts with `safe sign`, and executed with `safe execute`. The TUI "Safe multisig" menu deploys (ProxyFactory), imports, proposes, approves, and executes. Software, cloud, Ledger, and Trezor owners sign the EIP-712 SafeTx digest; the devices display the Safe domain/message hashes (or typed fields) for confirmation.
